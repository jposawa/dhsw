...
describe("equip", () => {
  it("recusa a segunda armadura, e devolve o nome da que ja esta vestida", () => {
    ...
    const result = equip(withInventory([worn, other]), other.id, TEST_COMPENDIUM)
  ...
  it("desequipar libera o slot para a proxima", () => {
    ...
    const next = unwrap(equip(freed, other.id, TEST_COMPENDIUM))
...
describe("equipInSlot", () => {
  ...
  it("troca com quem estava no slot, em uma operacao", () => {
    const other = createInventoryEntry("armor", "Scout Mesh")
    const next = unwrap(equipInSlot(withInventory([worn, other]), "armor", other.id, TEST_COMPENDIUM))
  ...
  it("id nulo esvazia o slot", () => {
    const next = unwrap(equipInSlot(withInventory([worn]), "armor", null, TEST_COMPENDIUM))

    expect(next.inventory[0].isEquipped).toBe(false)
  })

  it("recusa id que nao esta no inventario", () => {
    expect(equipInSlot(withInventory([worn]), "armor", "nao-existe", TEST_COMPENDIUM).ok).toBe(false)
  })
  ...
  it("nao mexe nos outros slots", () => {
    ...
    const next = unwrap(equipInSlot(withInventory([worn, blaster, other]), "armor", other.id, TEST_COMPENDIUM))
...
describe("burden — arma de duas maos ocupa as duas (p. 113)", () => {
  ...
  it("equip recusa secundaria com uma primaria de duas maos", () => {
    ...
    const result = equip(withInventory([rifle, knife]), knife.id, TEST_COMPENDIUM, "secondary")
  ...
  it("equipInSlot com duas maos na primaria manda a secundaria para a mochila", () => {
    ...
    const next = unwrap(equipInSlot(withInventory([knife, rifle]), "primary", rifle.id, TEST_COMPENDIUM))
  ...
  it("equipInSlot de secundaria tira a primaria de duas maos", () => {
    const rifle = equipped("Blaster Rifle", "primary")
    const knife = createInventoryEntry("weapon", "Vibroknife")
    const next = unwrap(equipInSlot(withInventory([rifle, knife]), "secondary", knife.id, TEST_COMPENDIUM))
  ...
  it("uma mao na primaria convive com a secundaria", () => {
    ...
    expect(equip(withInventory([pistol, knife]), knife.id, TEST_COMPENDIUM, "secondary").ok).toBe(true)
  })
})
