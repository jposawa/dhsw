...
export const CardsPanel = ({
	...
	const skillsOf = (names: readonly string[]): Skill[] =>
		names
			.map((name) =>
				compendium.skills.find((candidate) => candidate.name === name),
			)
	...
	const tokenPools = isEditing
		? []
		: activeTokenPools(character, derived, compendium);
	const tokensOf = (skill: Skill) =>
		tokenPools
			.filter(
				(active) => active.source === "card" && active.owner === skill.name,
			)
			.map((active) => (
				<TokenCounter
					key={active.key}
					...
					onChange={(next) =>
						onApply(
							setTokenCount(character, active.key, next, derived, compendium),
						)
					}
				/>
			));
			...
			<section className={styles.loadout}>
				...
				{loadout.length === 0 ? (
					<p className={styles.empty}>
						Loadout vazio. Traga uma carta do vault.
					</p>
				) : (
